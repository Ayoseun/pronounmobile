...
class PQActivity4 : AppCompatActivity(), View.OnClickListener {
    var mtimes = 0
    ...
    fun loadJson() {
        ...
        try {
            val inputStream: File? =
                this.getExternalFilesDir("Pronoun/Data/d.json")  //here is where we put the json file
...
//first time
    ...
    fun update(state: Int, score: String, total: String) {
        val thread = Thread {
            ...
            databaseHelper.updateData4(state, score, total)
    ...
    fun yes(score: String, total: String) {
        val thread = Thread {
            ...
            databaseHelper.insertData4(score, total)
