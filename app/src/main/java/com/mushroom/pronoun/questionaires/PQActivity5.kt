...
class PQActivity5 : AppCompatActivity(), View.OnClickListener {
    var mtimes = 0
    ...
    fun loadJson() {
        ...
        try {
            val inputStream: File? =
                this.getExternalFilesDir("Pronoun/Data/e.json")  //here is where we put the json file
...
//first time
    ...
    fun update(state: Int, score: String, total: String) {
        val thread = Thread {
            ...
            databaseHelper.updateData5(state, score, total)
    ...
    fun yes(score: String, total: String) {
        val thread = Thread {
            ...
            databaseHelper.insertData5(score, total)
